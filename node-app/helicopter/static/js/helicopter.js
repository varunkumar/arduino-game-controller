...
Heli.Consts = [
  {name: "State", consts: ["WAITING", "WAITING_USER", "PAUSED", "PLAYING", "DYING"]},
  {name: "Dir",   consts: ["UP", "DOWN"]},
  {name: "Mode",  consts: ["SINGLE", "CLIENT", "HOST"]},
];
...
Heli.Color = {
    ...
    USER        : "#AAAAAA", TARGET_STROKE : "#B24524",
...
Heli.Socket = function (params) {
    ...
    function init() {
        if (url != null) {
            ...
            msgHandler = ((msgHandler != null) ? msgHandler : function (data) { console.log("Msg Handler: " + data); });
            gameEventHandler = ((gameEventHandler != null) ? gameEventHandler : function (data) { console.log("Game Event Handler: " + data); });
            onHostReady = ((onHostReady != null) ? onHostReady : function (data) { console.log("Host Ready: " + data); }); 
            onDeclined = ((onDeclined != null) ? onDeclined : function (data) { console.log("Declined: " + data); });
            onReady = ((onReady != null) ? onReady : function (data) { console.log("Ready: " + data); });
            onControllerReady = ((onControllerReady != null) ? onControllerReady : function (data) { console.log("Controller Ready: " + data); });
...
Heli.User = function (params) {
    ...
    function reset(pos) {
        _distance = 0;
        _position = ((pos != null) ? pos : {x: 5, y:50});
        _trail = [];
        _momentum = 0;
        _thrusters = false;
        _acceleration = 0;
...
var HELICOPTER = (function() {
    ...
    function newGame(patterns) {
        if (state != Heli.State.PLAYING) { 
            user1.reset({x: 5, y: 50});
            user2.reset({x: 10, y: 50});

            // Positioning the host first
            if (mode === Heli.Mode.HOST) {
                user1.reset({x: 10, y: 50});
                user2.reset({x: 5, y: 50});
            }
    ...
    function init(wrapper, root, url) {
        ...
        user1 = new Heli.User({"tick": tick, "label": 1});
        user2 = new Heli.User({"tick": tick, "label": 2});
